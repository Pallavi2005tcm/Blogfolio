...
//   return (
//     <form onSubmit={onSubmitHandler} className='flex-1 bg-blue text-gray-600 h-full overflow-scroll'>
...
//           <select onChange={(e)=>setCategory(e.target.value)} name='category' className='mt-2 px-3 py-2 border text-gray-500 border-gray-300 otuline-none rounded'>
...
//             <p>Publish Now</p>
//             <input type='checkbox' checked={isPublished} className='scale-125 cursor-pointer' onChange={e=>setIsPublished(e.target.value)}></input>
//           </div>
...
const AddBlog = () => {
  ...
  const onSubmitHandler = async (e) => {
    try {
      ...
      const { data } = await axios.post('/api/blog/add', formdata,
      {
         headers: { 'Content-Type': 'multipart/form-data' } // Only content type
      })
  ...
  useEffect(() => {
    // ✅ EXACT SAME QUILL LOGIC AS YOURS
    if (!quillRef.current && editorRef.current) {
